...
class YouTubeUploader:
    ...
    def upload_from_metadata(self, metadata_path: str) -> str:
        ...
        # Try both 'video_path' and 'final_video_path' (pipeline uses 'final_video_path')
        video_path = metadata.get('video_path') or metadata.get('final_video_path')
        if not video_path or not os.path.exists(video_path):
            raise FileNotFoundError(f"Video file not found from metadata: {video_path}. Available keys: {list(metadata.keys())}")

        # Extract title and description from metadata - use KOREAN content
        # The pipeline now generates a Korean title in metadata['title']
        title = metadata.get('title', '오늘의 뉴스')  # Korean title generated by pipeline
        
        # Use description from metadata (also Korean), or Korean script as fallback
        description = metadata.get('description', '')
        if not description and metadata.get('korean_script'):
            # Fallback: use first part of Korean script
            korean_script = metadata.get('korean_script', '')
            first_paragraph = korean_script.split('\n\n')[0] if '\n\n' in korean_script else korean_script
            description = first_paragraph[:500]  # Limit length for YouTube

        # Add article source to description (in Korean/English mix is OK)
        if 'news_article' in metadata:
            article = metadata['news_article']
